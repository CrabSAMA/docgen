        ...
        propsRes.forEach((prop) => {
            ...
            for (let i = 0; i < propConfig.length; i++) {
                if (propConfig[i] === 'Name') {
                    ...
                    let desc = ['-'];
                    if (prop.describe.default && prop.describe.default.length) {
                        desc = prop.describe.default;
                        if (prop.validatorDesc) {
                            desc = prop.describe.default.concat(prop.validatorDesc);
                        }
                    }
                    row.push(desc.join(' '));
                }
                ...
                else if (propConfig[i] === 'Since') {
                    if (prop.describe.since) {
                        row.push(prop.describe.since[0])
                    }
                    else {
                        row.push('-');
                    }
                }
                else if (propConfig[i] === 'Value') {
                    if (prop.describe.values) {
                        row.push(prop.describe.values.join(','))
                    }
                    else {
                        row.push('-');
                    }
            ...
            for (let i = 0; i < eventConfig.length; i++) {
                if (eventConfig[i] === 'Event Name') {
                    ...
                    if (event.argumentsDesc) {
                        // author: crab.huang
                        // 添加数据类型
                        row.push(event.argumentsDesc.map(val => {
                            const type = `\`${val.match(/{(\S*)}/)[1]}\``;
                            return `${type} ${val.replace(/{(\S*)} /, '')}`
                        }).join(' '));
                    }
                    else {
                        row.push('-');
