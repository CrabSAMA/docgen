...
var Render = _interopDefault(require('../markdown-render'));
...
var genMarkdown = async (config) => {
    ...
    return files.map(async (p) => {
        ...
        try {
            ...
            if (!markdownRes)
                return;
            let str = markdownRes.content.replaceAll(/<!-- @vuese.*/g, '');
...
var preview = async (config) => {
    ...
    if (fs.existsSync(vueFile)) {
        ...
        const renderer = async () => {
            ...
            await page.evaluate((html, container) => (container.innerHTML = html), html, container);
        };
        // init
        renderer();
